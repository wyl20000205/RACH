...
class ImageResidualAttentionBlock(nn.Module):
    def __init__(
            self,
            d_model: int,
            n_head: int,
    ...
    def attention(
        ...
        return self.attn(
            q_x, k_x, v_x, key_padding_mask=key_padding_mask, need_weights=False, attn_mask=attn_mask
        )[0]
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, key_padding_mask=None):
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, image_prompt: torch.Tensor):
        ...
        # seq_tokens = x[1:]
        cls_token = x[0]
        return cls_token  # L(patches number)ND', NS(patches number), ND'
...
class TextTransformer(nn.Module):
    ...
    def forward(self, text, text_prompt):
        ...
        if self.cls_emb is not None:
            seq_len += 1
            x = torch.cat([x, text_prompt, _expand_token(self.cls_emb, x.shape[0])], dim=1)
        ...
        #     self._init_alibi_mul96(device=attn_mask.device)

        x = x + self.positional_embedding[:seq_len].to(cast_dtype)
        # if self.width > self.max_position_embeddings:
        #     self.max_position_embeddings = self.width

        # if self.rpe_type == "bipe_alibi":
        #     attn_weights = attn_weights + self.alibi[:, :, :, :self.width].squeeze().transpose(1,0)[self.num_pos].transpose(2,1).unsqueeze(2)
        # print(x.shape)
        # exit()
        x = x.permute(1, 0, 2)  # NLD -> LND
        ...
        # new_key_padding_mask = key_padding_mask + (text == 49407)  # mask global EOS token
